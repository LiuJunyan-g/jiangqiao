<template>
	<div>
		...
		<el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange">
			...
			<el-table-column label="商铺地址" width="120">
			...
			<el-table-column prop="store_type" label="类型" show-overflow-tooltip>
			</el-table-column>
					...
					<el-dialog title="商铺信息" :visible.sync="dialogFormVisible">
						<el-form :model="form">
							...
							<el-form-item label="类型" :label-width="formLabelWidth">
								<el-input v-model="form.store_type" autocomplete="off" @change="landsource_change"></el-input>
							</el-form-item>
		...
		methods: {
			...
			change_landsoureinfo() {
				if (this.landsoure == "1") {
					...
					this.$axios({
						method: "PUT",
						url: "http://192.168.20.67:8001/api/update_store_resource/" + this.form.id + "/",
						data: this.data,
					}).then(res => {
						location.reload()
					})
			...
			handleDelete(index) {
				...
				this.$axios({
					method: "DELETE",
					url: "http://192.168.20.67:8001/api/delete_store_resource/" + this.tableData[index].id + "/",
					data: {
						user_id: localStorage.getItem("user_id")
					}
				}).then(res => {
			...
			handleCurrentChange(val) {
				...
				this.$axios({
					method: "GET",
					url: "http://192.168.20.67:8001/api/get_store_resources/?user_id=" + localStorage.getItem("user_id") + "&" +
						"page=" + _this.page,
				}).then(res => {
