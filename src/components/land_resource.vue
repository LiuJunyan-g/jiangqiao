<template>
	<div>
		...
		<el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange">
			...
			<el-table-column label="土地所在地址" width="120">
				<template slot-scope="scope">{{ scope.row.location }}</template>
			</el-table-column>
			<el-table-column prop="crops" label="所种农作物" width="120">
			</el-table-column>
			<el-table-column prop="floor_space" label="占地面积" show-overflow-tooltip>
			</el-table-column>
			<el-table-column prop="max_height" label="建筑限高" show-overflow-tooltip>
			</el-table-column>
					...
					<el-dialog title="土地信息" :visible.sync="dialogFormVisible">
						<el-form :model="form">
							...
							<el-form-item label="所种农作物" :label-width="formLabelWidth">
								<el-input v-model="form.crops" autocomplete="off" @change="landsource_change"></el-input>
							</el-form-item>
		...
		methods: {
			...
			change_landsoureinfo() {
				if (this.landsoure == "1") {
					...
					this.$axios({
						method: "PUT",
						url: "http://192.168.20.67:8001/api/update_land_resource/" + this.form.id + "/",
						data: this.data,
					}).then(res => {
						location.reload()
					})
			...
			handleDelete(index) {
				...
				this.$axios({
					method: "DELETE",
					url: "http://192.168.20.67:8001/api/delete_land_resource/" + this.tableData[index].id + "/",
					data: {
						user_id: localStorage.getItem("user_id")
					}
				}).then(res => {
